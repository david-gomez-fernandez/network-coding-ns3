...
#ifndef INTER_FLOW_NETWORK_CODING_BUFFER_H_
#define INTER_FLOW_NETWORK_CODING_BUFFER_H_
...
class InterFlowNetworkCodingBuffer: public Object
{
public:
	 ...
	 * Default constructor
	 */
	InterFlowNetworkCodingBuffer ();
	/**
	 * Default destructor
	 */
	virtual ~InterFlowNetworkCodingBuffer ();
	 ...
	 */
	bool UpdateInputPacketPool (Ptr <Packet> packet, InterFlowNetworkCodingHeader ncHeader, Ipv4Address source, Ipv4Address destination, u_int8_t protocol);
	...
	////ACK encapsulation-related feature
	/*
	 ...
	 */
	bool EncapsulateTcpAckSegments (InterFlowNetworkCodingHeader &ncHeader, u_int16_t packetLength);

	////End of ACK encapsulation-related feature
	...
	/**
	 * Test function to handle the TcpRxBuffer's next sequence number expected
	 */
	void TcpSequenceNumber (std::string context, SequenceNumber32 seqNum);
...
private:
	...
	Time m_ncBufferTimeout;								//Time interval while the buffer keep the overheard packets
	Time m_decodingBufferStoreTime;						//Time during which the decoding buffer will hold on a native packet
	Time m_ackBufferStoreTime;							//Time interval during while the buffers can keep an ACK
	...
	SequenceNumber32 m_tcpSequenceNumber;
};

}  //End namespace
#endif /* INTER_FLOW_NETWORK_CODING_BUFFER_H_ */
