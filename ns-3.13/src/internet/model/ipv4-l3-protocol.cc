...
namespace ns3 {
    ...
    Ipv4L3Protocol::IpForward(Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header) {
        ...
        ////David/Ramón --> IP Forwarding: We need to intercept the Network Coding packets, in order not to replicate the information delivery.
        //Discard all the Network Coding packets (The NetworkCodingL4Protocol overhearing scheme will take care of them)
        //  99 -> InterFlowNetworkCodingProtocol (TCP and UDP)
        // 100 -> InterFlowNetworkCodingProtocol (UDP only)

        if ((ipHeader.GetProtocol() == 99) || (ipHeader.GetProtocol() == 100))
        {
        	//Connect to the Network Coding layer forwarding handler
        	if (!m_ipForwardingCallback.IsNull())
        		m_ipForwardingCallback (rtentry, p, header);

            NS_LOG_LOGIC("Network coding packet intercepted. The NetworkCodingL4Protocol will be in charge of forwarding it");
            return;
        }
        ////End David/Ramón  
