...
void HiddenMarkovModelEntry::MapFerValue (double fer)
{
	...
	{
		transitionMatrixFileName = "HMM_4states/HMM_13_TR_6.txt";
		emissionMatrixFileName = "HMM_4states/HMM_13_EMIS_6.txt";
	}
	...
	else if (fer < 0.30)  	//Load average channel Coeffs
	{
		transitionMatrixFileName = "HMM_4states/HMM_09_TR_3.txt";
		emissionMatrixFileName = "HMM_4states/HMM_09_EMIS_3.txt";
	}
	else if (fer < 0.40)  	//Load average channel Coeffs
	{
		transitionMatrixFileName = "HMM_4states/HMM_07_TR_6.txt";
		emissionMatrixFileName = "HMM_4states/HMM_07_EMIS_6.txt";
	}
	else if (fer < 0.60)	//Load Bad Channel Coefficients
	{
		transitionMatrixFileName = "HMM_4states/HMM_05_TR_6.txt";
		emissionMatrixFileName = "HMM_4states/HMM_05_EMIS_6.txt";
	}
	else if (fer < 0.75)	//Load Bad Channel Coefficients
	{
		transitionMatrixFileName = "HMM_4states/HMM_02_TR_3.txt";
		emissionMatrixFileName = "HMM_4states/HMM_02_EMIS_3.txt";
	}
	else if (fer < 0.85)	//Load Bad Channel Coefficients
	{
		transitionMatrixFileName = "HMM_4states/HMM_01_TR_1.txt";
		emissionMatrixFileName = "HMM_4states/HMM_01_EMIS_1.txt";
	}
	else
	{
...
void HiddenMarkovModelEntry::MapDistanceValue (double distance)
{
	...
	if (distance < 9)			//Load idealchannel
	{
		transitionMatrixFileName = "HMM_4states/Ideal_TR.txt";
		emissionMatrixFileName = "HMM_4states/Ideal_EMIS.txt";
	}
	else if (distance < 13)  	//Load good channel Coeffs
	{
		transitionMatrixFileName = "HMM_4states/HMM_13_TR_6.txt";
		emissionMatrixFileName = "HMM_4states/HMM_13_EMIS_6.txt";
	}
	else if (distance < 17)  	//Load good channel Coeffs
	{
		transitionMatrixFileName = "HMM_4states/HMM_12_TR_1.txt";
		emissionMatrixFileName = "HMM_4states/HMM_12_EMIS_1.txt";
	}
	else if (distance < 22)  	//Load average channel Coeffs
	{
		transitionMatrixFileName = "HMM_4states/HMM_09_TR_3.txt";
		emissionMatrixFileName = "HMM_4states/HMM_09_EMIS_3.txt";
	}
	else if (distance < 27)	//Load Bad Channel Coefficients
	{
		transitionMatrixFileName = "HMM_4states/HMM_05_TR_6.txt";
		emissionMatrixFileName = "HMM_4states/HMM_05_EMIS_6.txt";
	}
...
bool HiddenMarkovModelEntry::GetCoefficients (string transitionMatrixFileName, string emissionMatrixFileName)
{
	...
	double aux = 0.0;															//Auxiliary variable used to accumulate the
	int  rowNumber,i,j, counter = 0, states = 0;												//Auxiliary counters
	double coefficient;
	...
	{
		if (rowNumber == 0)
		{ 											//First item in file--> Number of states in the Hidden Markov Chain
			states = atoi (line);
		}
		else
		{ 														//Rest of values are the coefficient of the channel model
			j = 0;
			for (i = 0; i < states; i++)
			{
				...
				{
					while (line[j] != ' ' && line[j] != '\t')
					{ 		//Look for "white" spaces between Coefficients
						j++;
					}
	...
	while (emissionMatrixFile.getline(line, 256))
	{
		j = 0;
		for (i = 0; i < 2; i++)
		{			//Two will always be the number of hidden states
			//First value at position '0'
			if (i == 0)
			{
				coefficient = atof(line);
	...
	//Update the matrix probabilities
	for (map<int, vector<double> >::iterator iter = m_transitionMatrix.begin(); iter != m_transitionMatrix.end(); iter ++)
	{
		vector <double> coefficientsVector;
		for (i = 0; i < (int) iter->second.size(); i++)
		{
			...
			else
			{
				if (i != counter)
				{
					aux += iter->second[i] / (1 - (iter->second)[counter]);
				}

			}
//			cout << aux << endl;
			coefficientsVector.push_back(aux);
		}

		m_changeStateMatrix.insert(pair<int, vector <double> > (counter, coefficientsVector));   //Insert the coefficient vector
	...
	//Ensure that the matrices have been successfully generated (its size its different than zero)
...
//	double transitionProbability, max, randomSample;
	...
	for (i = 0; (int) i < (m_changeStateMatrix [m_currentState]).size(); i++)
	{
		if (randomSample < (m_changeStateMatrix [m_currentState])[i])  //Change state condition
		{
			if (m_currentState != i)
			{
				if (m_debugState)
				{
					cout << Simulator::Now().GetSeconds() << " " << (int) m_currentState << " " << (int) i << endl;
				}
				m_currentState = i;
			}
			break;
...
//			}
//		}
//		else				//Frame-based characterization
//		{
...
//		if (m_debugState)
//		{
//			cout << Simulator::Now().GetSeconds() << " " << (int) m_currentState << " " << (int) maxState << endl;
//		}
...
void HiddenMarkovModelEntry::PrintMatrices ()
{
	...
	printf ("---Average time duration at state i---\n");
	for (j = 0; j < (int) m_averageInterFrameTime.size(); j++ )
	{
		printf ("State %d --> Average time = %.4f microseconds\n", j, m_meanDurationVector[j] *m_averageInterFrameTime[j]);
	}
